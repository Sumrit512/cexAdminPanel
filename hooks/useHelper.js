...
export const formatText = ( data, len) => {
    try {
            
        if(data.length > 10 && len < 6) {
            const formattedText = `${data.substring(0, len)}...${data.substring(data.length- 3, data.length )}`;
        ...
        console.log("[FORMAT_TEXT_HOOK]",e)
    }
} 